...
SELECT DISTINCT
    ...
    nome_depto
FROM
    depto JOIN professor ON depto.cod_depto = professor = cod_depto
...
SELECT DISTINCT
    nome_depto
FROM
    depto JOIN professor ON depto.id_depto = professor.id_depto
...
SELECT 
    ...
    nome_peca
FROM 
    embarque JOIN fornecedor ON embarque.cod_fornec = fornecedor.cod_fornec
...
SELECT DISTINCT
    ...
    nome_depto
FROM
    depto JOIN professor USING (cod_depto)
...
SELECT 
    ...
    nome_peca
FROM 
    embarque JOIN fornecedor USING (cod_fornec)
...
SELECT DISTINCT
    nome_prof
FROM
    professor JOIN prof_oferta USING (cod_prof)
    JOIN oferta USING (sigla_ofer)
WHERE
    ano_sem = '20022';
...
SELECT
    ...
    sigla_ofer
FROM
    depto LEFT JOIN disciplina ON (depto.cod_depto = disciplina.cod_depto)
...
FROM
    disciplina LEFT JOIN prereq ON disciplina.num_disc = prereq.num_disc;
